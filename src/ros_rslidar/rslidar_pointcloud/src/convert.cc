...
void pointProcess(Point_process &pp);
...
void Convert::processScan(const rslidar_msgs::rslidarScan::ConstPtr& scanMsg)
{
  ...
  Point_process pp(outPoints);
  rslidar_pointcloud::pointProcess(pp);
...
{
  pp.cleanDownPoints();
  pp.voxelFilter(); //体素滤波器 对点云下采样
  //pp.radiusOutlierRemoval(); //根据半径范围内的点进行滤波，此次不使用
  //pp.filter();         //DoF算法
  //pp.segmentGround();
  pp.segmentPoints(); 
